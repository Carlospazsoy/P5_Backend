...
const { sendEmail } = require("../libs/email"); // importa configuración de sendgrid, sendmail es ahora utlizable como un metodo sendEmail(valor1,valor2,valor3) se reelenara con valores que pueden o no venir del req.body
...
const userById = async (req, res) => {
  try {
    const userId = req.params.id; // Extrae el id de los parámetros de la solicitud
    const oneUser = await User.findById(userId).populate('addresses'); // Usa el id para buscar el usuario
    
    if (!oneUser) { // Si no se encuentra el usuario, envía un mensaje de error
      return res.status(404).json({
        ...
        return res.status(200).json({
          success: true,
          // firstAddress: oneUser.addresses[0],
          data: oneUser,
    });
  } catch (error) {
    return res.status(400).json({
...
const signup = async (req, res) => {
  try {
    ...
    console.log("Error de validación si es undefined todo bien: ", validationResult?.errors)
...
const recover =(req, res) => {
  /* auth es un metodo de  express-jwt lo encontro en la documentacion , muestra los credenciales del token S22 dentro de un objeto 3:18:00*/
  // console.log(req); 
  return res.json({
    success:true,
    data:req.auth
...
const updateRole = async (req, res) => {
  try {
   ...
   /*  if (req.auth.role !== "ADMIN") {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId,{ role: role },{ new: true } ); // Devuelve el documento actualizado
    ...
    return res.status(500).json({
      success: false,
      message: "Error interno del servidor o tu id no se encontró",
      error: error.message,
    });
  }
};
...
const getPreferredAddress = async (req, res) => {
  try {
    const userId = req.params.id;

    // Encuentra el usuario por ID y popula el campo preferredAddress
    ...
    // Verifica si hay un domicilio preferido
    if (!user.preferredAddress) {
      return res.status(404).json({
        success: false,
        message: "Usuario no tiene un domicilio preferido establecido",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      data: user.preferredAddress,

    });
  } catch (error) {
    return res.status(400).json({
      success: false,
      message: "Error al obtener el domicilio preferido",
      error: error.message,
    });
  }
};

const setPreferredAddress = async (req, res) => {
  try {
    const userId = req.params.id; // Pasado en la ruta, definido en las rutas
    const { addressId } = req.body;

    // Busca al documento usuario dentro de la colección users asociada al modelo User mediante el Id
    ...
    // Actualiza el campo preferredAddress del documento del usuario
    user.preferredAddress = addressId;
    // Guarda los cambios en la base de datos
    await user.save();

    // Consulta la dirección preferida para obtener los detalles poblados utilizando el modelo Address
    const preferredAddress = await Address.findById(addressId);

    return res.status(200).json({
      success: true,
      message: `Domicilio preferido del usuario: ${user.name} ${user.lastName} ha sido actualizado`,
      preferredAddress: preferredAddress // Devuelve los detalles de la dirección preferida
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
      message: "Error al actualizar el domicilio preferido",
      error: error.message,
    });
  }
};
