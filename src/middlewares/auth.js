...
const verifyRole = (req, res, next ) => {
  ...
  if (req.auth.role!=="ADMIN") {
    return res.status(403).json({
      succes: false,
      message: "No tienes el rol necesario para realizar este cambio según el middleware"
    })  
  }
  return next()
}
...
const verifyAllRoles = (allowedRoles) => { //allowedRoles es el array de roles que le pasamos en rutas y lo compara con los roles definidos en el modelo
